...
export const applicantsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/screen/platform", { config: { rateLimit: { max: 10, timeWindow: "1 hour" } } }, screenPlatformApplicants);
  app.post("/screen/external", { config: { rateLimit: { max: 10, timeWindow: "1 hour" } } }, screenExternalApplicants);
