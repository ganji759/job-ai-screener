...
export const FileDropzone = ({
  ...
  return (
    <div
      {...getRootProps()}
      className={cn(
        "rounded-2xl border-2 border-dashed p-8 text-center transition outline-none",
    ...
    >
      ...
      <Upload className="mx-auto mb-2 h-8 w-8 text-brand-500 dark:text-brand-400" aria-hidden />
      <p className="text-sm font-medium text-slate-700 dark:text-slate-200">
        {disabled ? "Upload disabled" : isDragActive ? "Drop files here…" : "Drag & drop files here, or click to browse"}
      </p>
      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">PDF, CSV, or Excel depending on your selection</p>
    </div>
  );
};
